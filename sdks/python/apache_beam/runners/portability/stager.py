...
def retry_on_non_zero_exit(exception):
    if (
        isinstance(exception, processes.CalledProcessError)
...
class Stager(object):
    ...
    def create_job_resources(
        ...
        use_beam_default_container = (
            options.view_as(WorkerOptions).sdk_container_image is None
        )
            ...
            if setup_options.requirements_file is not None:
                ...
                if extra_packages:
                    setup_options.extra_packages = (
                        setup_options.extra_packages or []
                    ) + extra_packages
                resources.append(
                ...
                if setup_options.requirements_cache != SKIP_REQUIREMENTS_CACHE:
                    (
                        ...
                        if populate_requirements_cache
                        else Stager._populate_requirements_cache
                    )(
                        setup_options.requirements_file,
                        requirements_cache_path,
            ...
            if hasattr(setup_options, "sdk_location"):
                ...
                if Stager._is_remote_path(sdk_location):
                    ...
                    if os.path.isfile(sdk_path):
                        _LOGGER.info(
                            'Copying Beam SDK "%s" to staging location.', sdk_path
                        )
                        resources.append(
                            Stager._create_file_stage_to_artifact(
                                sdk_path,
        ...
        if jar_packages is not None:
            resources.extend(
                Stager._create_jar_packages(jar_packages.split(","), temp_dir=temp_dir)
            )
    ...
    def _create_extra_packages(extra_packages, temp_dir):
        ...
        for package in extra_packages:
            if not (
                os.path.basename(package).endswith(".tar")
                or os.path.basename(package).endswith(".tar.gz")
                or os.path.basename(package).endswith(".whl")
                or os.path.basename(package).endswith(".zip")
            ):
                raise RuntimeError(
    ...
    def _extract_local_packages(requirements_file):
        ...
        if local_deps:
            with tempfile.NamedTemporaryFile(
                suffix="-requirements.txt", delete=False
            ) as fout:
