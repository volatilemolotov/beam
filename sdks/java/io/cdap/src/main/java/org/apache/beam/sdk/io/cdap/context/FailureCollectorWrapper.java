...
public class FailureCollectorWrapper implements FailureCollector {
  ...
  public ValidationException getOrThrowException() throws ValidationException {
    ...
    //We skip schema field validation errors because they are CDAP oriented and don't affect anything in our case
    for (Iterator<ValidationFailure> iterator = failuresCollection.iterator();iterator.hasNext();) {
      String inputField = iterator.next().getCauses().get(0).getAttribute(CauseAttributes.INPUT_SCHEMA_FIELD);
