...
public class SendGridSinkConfigValidator extends BaseConfigValidator {
  ...
  /**
   * The regular expression to check if string matches to "part1 @ part2 . part3" format. The
   * details for each part as, 1. ^[\w\d.+\-]+ = Beginning of the string should have at least one
   * word character or digit character or plus or hyphen or dot 2. [\w\d]+ = at least one word
   * character or digit character 3. [\w\d]+$ = end with at least one word character or digit
   * character
   */
  ...
  private void checkFrom() {
    if (Strings.isNullOrEmpty(config.getFrom()) || !validateEmail(config.getFrom())) {
      failureCollector
          .addFailure("Not a valid or empty email address", null)
  ...
  private void checkFooter() {
    if (config.getFooterEnable() && Strings.isNullOrEmpty(config.getFooterHTML())) {
      failureCollector
          .addFailure("Footer content cannot be empty", null)
