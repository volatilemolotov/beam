...
public class ZuoraSinkConfig extends BaseConfig {
      ...
      switch (toType) {
        ...
        default:
          throw new IllegalArgumentException(
              String.format(
                  "Unknown arguments source '%s', allowed: '%s', '%s'",
  ...
  @Description(
      "Object arguments would be taken from the sink configuration or input schema columns")
  @Macro
  private String requestArgumentsSource;

  @Name(PROPERTY_REQUEST_ARGUMENTS)
  @Description(
      "Arguments required for the object. If input record request arguments selected, contains only names")
  @Nullable
  @Macro
  ...
  private void validateField(Schema schema, String fieldName) {
    ...
    if (field == null) {
      throw new IllegalArgumentException(
          String.format(
              "Plugin is configured to use column '%s'"
                  + ", but input schema did not provide such column",
              fieldName));
    }
    ...
    if (fieldSchema.getType() == Schema.Type.UNION) {
      if (fieldSchema.getUnionSchemas().stream()
        ...
        throw new IllegalArgumentException(
            String.format("The input schema column '%s' expected to be of type STRING", fieldName));
      }
      return;
    }
  ...
  public Map<String, String> getRequestArguments() {
    ...
    if (!Strings.isNullOrEmpty(requestArguments)) {
      Arrays.stream(requestArguments.split(","))
          .forEach(
              x -> {
                String[] pair = x.split(":");
                if (pair.length == 2) {
