...
public class SendGridSourceConfigValidator extends BaseConfigValidator {
  private static Pattern datePattern =
      Pattern.compile("(?<year>\\d{4})-(?<month>\\d{2})-(?<day>\\d{1,2})");
  private SendGridSourceConfig config;

  public SendGridSourceConfigValidator(
      FailureCollector failureCollector, SendGridSourceConfig config) {
  ...
  private void checkCategoriesSelection() {
    if (config.getDataSourceTypes().isEmpty()) {
      failureCollector
          .addFailure("Object categories are not set", null)
  ...
  private void checkObjectsSelection() {
    ...
    List<DataSourceGroupType> categories =
        config.getDataSourceTypes().stream()
    ...
    categories.forEach(
        category -> {
          if (objects.stream().noneMatch(x -> x.getDataSourceGroupType() == category)) {
            failureCollector
                .addFailure(
                    String.format("No objects selected for the category: %s", category.name()),
                    null)
    ...
    objects.forEach(
        object -> {
          if (object.getFieldsDefinitions(fields).isEmpty()) {
            failureCollector
                .addFailure(
                    String.format("No fields selected for object '%s'", object.getCdapObjectName()),
                    null)
  ...
  private void checkRequiredInputProperties() {
    getObjectsStream()
        ...
        .forEach(
            arg ->
                failureCollector
                    .addFailure(String.format("Argument %s cannot be empty", arg), null)
  ...
  private void checkDateFormat(String date, String field) {
    if (!Strings.isNullOrEmpty(date)) {
      ...
      if (!matcher.find()) {
        failureCollector
            .addFailure("Input format should math YYYY-MM-DD", null)
        ...
        if (month < 1 || month > 12) {
          failureCollector
              .addFailure(
                  "Input format should math YYYY-MM-DD and MM should be in range from 1 to 12",
                  null)
              .withConfigProperty(field);
        }
        ...
        if (day < 1 || day > 31) {
          failureCollector
              .addFailure(
                  "Input format should math YYYY-MM-DD and DD should be in range from 1 to 31",
                  null)
              .withConfigProperty(field);
        }
      }
