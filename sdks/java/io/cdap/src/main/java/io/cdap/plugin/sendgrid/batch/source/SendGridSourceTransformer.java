...
public class SendGridSourceTransformer {
  ...
  private static void transformValue(
      String k, Object v, Schema schema, StructuredRecord.Builder builder) {
    ...
    if (v instanceof Map) {
      ...
      Schema componentSchema =
          Objects.requireNonNull(schema.getField(k)).getSchema().getComponentSchema();
      if (componentSchema == null) {
        throw new IllegalArgumentException(
            String.format("Unable to extract schema for the field '%s'", k));
      }
      Object values =
          ((List) v)
              .stream()
                  .map(arrItem -> transform((Map<String, Object>) arrItem, componentSchema))
  ...
  public static StructuredRecord transform(IBaseObject object, Schema schema) {
    ...
    object.asFilteredMap(schema).forEach((k, v) -> transformValue(k, v, schema, builder));
