...
func Process(ctx context.Context, cacheService cache.Cache, lc *fs_tool.LifeCycle, pipelineId uuid.UUID, appEnv *environment.ApplicationEnvs, sdkEnv *environment.BeamEnvs, pipelineOptions string, mockCluster emulators.EmulatorMockCluster, prepareParams map[string]string) {
	...
	err := validateStep(pipelineLifeCycleCtx, cacheService, &lc.Paths, pipelineId, sdkEnv, &validationResults, cancelChannel)
	if err != nil {
		return
	}

	executor := prepareStep(pipelineLifeCycleCtx, cacheService, &lc.Paths, pipelineId, sdkEnv, &validationResults, cancelChannel, prepareParams)
	if executor == nil {
		return
	}
...
func validateStep(ctx context.Context, cacheService cache.Cache, paths *fs_tool.LifeCyclePaths, pipelineId uuid.UUID, sdkEnv *environment.BeamEnvs, validationResults *sync.Map, cancelChannel chan bool) error {
	errorChannel, successChannel := createStatusChannels()
	...
	for {
		...
		case <-ticker.C:
			if _, err := os.Stat(graphFilePath); err == nil {
				...
				_ = utils.SetToCache(cacheService, pipelineId, cache.Graph, graph)
			}
