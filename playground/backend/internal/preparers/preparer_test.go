...
const (
	...
	correctGoTestFile = "correct_test.go"
...
func teardown() {
	files := []string{correctGoFile, incorrectGoFile, correctPyFile, incorrectPyFile, pyGraphFile, correctGoTestFile,
...
func TestGetPreparers(t *testing.T) {
	filepath := ""
	valResults := sync.Map{}
	valResults.Store(validators.UnitTestValidatorName, false)
	valResults.Store(validators.KatasValidatorName, false)

	valResultsWithoutKatas := sync.Map{}
	valResultsWithoutKatas.Store(validators.UnitTestValidatorName, false)
	...
	tests := []struct {
		...
		{
			name: "Get preparers without validation results",
			args: args{
				sdk:        pb.Sdk_SDK_JAVA,
				filepath:   filepath,
				valResults: &sync.Map{},
		...
		{
			name: "Get Java preparers without information about katas validation result",
			args: args{
				sdk:        pb.Sdk_SDK_JAVA,
				filepath:   filepath,
				valResults: &valResultsWithoutKatas,
			},
			want:    nil,
			wantErr: true,
		},
		{
			name: "Get preparers with unspecified sdk",
			args: args{
