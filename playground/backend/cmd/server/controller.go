...
func (controller *playgroundController) RunCode(ctx context.Context, info *pb.RunCodeRequest) (*pb.RunCodeResponse, error) {
	...
	if info.Sdk != controller.env.BeamSdkEnvs.ApacheBeamSdk {
		...
		return nil, cerrors.InvalidArgumentError(errorTitleRunCode, "Incorrect sdk. Want to receive %s, but the request contains %s", controller.env.BeamSdkEnvs.ApacheBeamSdk.String(), info.Sdk.String())
	}
	switch info.Sdk {
	case pb.Sdk_SDK_UNSPECIFIED:
		logger.Errorf("RunCode(): unimplemented sdk: %s\n", info.Sdk)
		return nil, cerrors.InvalidArgumentError(errorTitleRunCode, "Sdk is not implemented yet: %s", info.Sdk.String())
	}
	...
	if err != nil {
		...
		return nil, cerrors.InternalError(errorTitleRunCode, "Error during setup file system for the code processing: %s", err.Error())
	}
	...
	if err = utils.SetToCache(ctx, controller.cacheService, pipelineId, cache.Status, pb.Status_STATUS_VALIDATING); err != nil {
		...
		return nil, cerrors.InternalError(errorTitleRunCode, "Error during saving status of the code processing")
	}
	if err = utils.SetToCache(ctx, controller.cacheService, pipelineId, cache.RunOutputIndex, 0); err != nil {
		code_processing.DeleteResources(pipelineId, lc, kafkaMockCluster)
		return nil, cerrors.InternalError(errorTitleRunCode, "Error during saving initial run output")
	}
	if err = utils.SetToCache(ctx, controller.cacheService, pipelineId, cache.LogsIndex, 0); err != nil {
		code_processing.DeleteResources(pipelineId, lc, kafkaMockCluster)
		return nil, cerrors.InternalError(errorTitleRunCode, "Error during saving value for the logs output")
	}
	if err = utils.SetToCache(ctx, controller.cacheService, pipelineId, cache.Canceled, false); err != nil {
		code_processing.DeleteResources(pipelineId, lc, kafkaMockCluster)
		return nil, cerrors.InternalError(errorTitleRunCode, "Error during saving initial cancel flag")
	}
	if err = controller.cacheService.SetExpTime(ctx, pipelineId, cacheExpirationTime); err != nil {
		...
		code_processing.DeleteResources(pipelineId, lc, kafkaMockCluster)
		return nil, cerrors.InternalError(errorTitleRunCode, "Internal error")
	}
