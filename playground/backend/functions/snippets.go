...
func cleanupSnippets(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		log.Printf("Couldn't create the database client, err: %s\n", err.Error())
		// Return 500 error
	...
	if err != nil {
		log.Printf("Couldn't delete unused code snippets, err: %s \n", err.Error())
		// Return 500 error and error message
		w.WriteHeader(http.StatusInternalServerError)
		_, werr := w.Write([]byte(err.Error()))
		if werr != nil {
			log.Printf("Couldn't write error message, err: %s \n", werr.Error())
		}
		return
	}
...
func deleteObsoleteSnippets(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("Couldn't delete obsolete code snippets for snipId %s, persistenceKey %s, err: %s \n", snipId, persistenceKey, err.Error())
		// Return 500 error and error message
		w.WriteHeader(http.StatusInternalServerError)
		_, werr := w.Write([]byte(err.Error()))
		if werr != nil {
...
func incrementSnippetViews(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("Couldn't increment snippet visitors count for snipId %s, err: %s \n", snipId, err.Error())
		// Return 500 error and error message
		w.WriteHeader(http.StatusInternalServerError)
		_, werr := w.Write([]byte(err.Error()))
		if werr != nil {
