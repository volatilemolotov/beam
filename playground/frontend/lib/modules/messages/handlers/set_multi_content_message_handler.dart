...
class SetMultiContentMessageHandler extends AbstractMessageHandler {
  final PlaygroundState playgroundState;

  SetMultiContentMessageHandler({
    required this.playgroundState,
  });

  @override
  MessageHandleResult handle(AbstractMessage message) {
    if (message is! SetMultiContentMessage) {
  ...
  void _handle(SetMultiContentMessage message) {
    message.content.forEach(_handleSingle);
  }

  void _handleSingle(SetContentMessage message) {
    ...
    if (content != null) {
      playgroundState.setContentForSdk(sdk, content);
    }
  }
}
